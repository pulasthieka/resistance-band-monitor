            ...
            while obj.hUDP.BytesAvailable > 0
                ...
                switch RecvData(1)
                    ...
                    case 'S'
                        ...
                        obj.LiveYlims = [obj.Low_lim obj.High_lim]*obj.calibration_const/obj.ADCgain;
        ...
        function [obj, hFig] = plotLive(obj, RecordTime)
            if obj.Connected                
                ...
                if isempty(which('butter')) == false
                    if length(obj.freqBandpass) == 1
                        [b_BandPass, a_BandPass] = butter(4,obj.freqBandpass/obj.ADCsamplerate*2,'low');
                    elseif length(obj.freqBandpass) == 2
                ...
                while (RecordTime <= 0  || toc < RecordTime) && ishandle(hFig)
                    ...
                    if nRecvDataPackets > 0 && ishandle(hFig)
                        ...
                        if hChkUpdateData.Value && UpdatePlot
                            ...
                            % Update the plot
                            for iInput = 1:obj.nADCinput
                                set(hLines(iInput),'XData', obj.TimeAxis(iLiveBuffer(iLiveWindow)), 'YData', ((obj.scale_new)*(LiveBuffer(iInput,iLiveWindow))+obj.offset_new));
                            end
